...
> g2p.py를 which로 찾지 못하여,
>
> python $KALDI_ROOT/tools/sequitur-g2p/g2p.py로 코드 변경
...
if [ $stage -le 0 ]; then
  $cmd $wdir/log/g2p.0.log \
    python $KALDI_ROOT/tools/sequitur-g2p/g2p.py -S --encoding $encoding --train $lexicon --devel 5% --write-model $wdir/g2p.model.0
fi
...
for i in `seq 0 $(($iters-2))`; do
  ...
  if [ $stage -le $i ]; then
    $cmd $wdir/log/g2p.$(($i + 1)).log \
      python $KALDI_ROOT/tools/sequitur-g2p/g2p.py -S --encoding $encoding --model $wdir/g2p.model.$i --ramp-up --train $lexicon --devel 5% --write-model $wdir/g2p.model.$(($i+1))
  fi
...
if [ $stage -le $(($i + 2)) ]; then
  ...
  $cmd $wdir/log/test.log \
    python $KALDI_ROOT/tools/sequitur-g2p/g2p.py --encoding $encoding --model $wdir/g2p.model.final --test $lexicon
fi
